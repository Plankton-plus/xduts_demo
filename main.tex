\documentclass{xduugthesis}
\usepackage{fontspec}
\setmainfont{Times New Roman}
\usepackage{float}
\usepackage{unicode-math}
\setmathfont{XITS Math}
\usepackage{titlesec}
\titleformat{\section}[block]
  {\bfseries\fontsize{14pt}{16.8pt}\selectfont\centering}
  {\thesection}{1em}{}
\titleformat{\subsection}[block]
  {\bfseries\fontsize{12pt}{14.4pt}\selectfont}
  {\thesubsection}
  {1em}
  {}

\usepackage{amsthm}   % 定理类环境
\usepackage{amsmath}  % 数学符号
\usepackage{amssymb}  % 可选

% 定义 definition 环境（带编号）
\newtheorem{definition}{定义}    % 提供 \mathbb 命令
\usepackage{booktabs}
\usepackage{array}
\usepackage{graphicx}
\xdusetup{
    style = {
    cjk-font = fandol,
    math-font = xits,
    latin-font = tcc,
    unicode-math = {},
    font-type = font,
    font-path = fonts
  },
  info = {
    title = {轻量化密态区块链数据库\\可验证查询方法设计与实现},
    author = {杨芸茜},
    department = {计算机科学与技术学院},
    major = {计算机科学与技术},
    class-id = {2203013},
    student-id = {22009201086},
    supervisor = {王运帷},
    bib-resource = {ref.bib},
    abstract = {chapters/abstract_zh.tex},
    abstract* = {chapters/abstract_en.tex},
    keywords = {牛马动力学,工程创新,工程伦理,施工方法,工程质量控制},
    keywords* = {Animal Dynamics, Engineering Innovation, Engineering Ethics, Construction Methods, Engineering Quality Control},
    acknowledgements = {chapters/acknowledge.tex},
    bib-resource = reference.bib
    }
  }
\begin{document}

% 临时解决\cite{}没有自动补全提示的问题
% 详见 https://github.com/note286/xduts/discussions/157
\iffalse
  \addbibresource{ref.bib}
\fi


\include{chapters/intro}
\include{chapters/preliminaries}
\include{chapters/system_model}
\include{chapters/system_init}
\include{chapters/index_buid}
\include{chapters/cipher_query_verify}
\include{chapters/conclusion}


\end{document}